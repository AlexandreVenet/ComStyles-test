...
// version initiale du script avec des setTimeout
...
class Test{
	...
	lancerTest(){
		...
		setTimeout(()=>{ // après avoir laissé le temps de disparition
			this.intro.remove();
			this.nouvelleEtape();
		},310); // 300ms + marge
	}
	...
	nouvelleEtape(){
		...
		this.aide.innerHTML = `${numeroGroupeStr} / ${this.totalGroupes}`;
	...
	clickLien(event){
		...
		// après un certain temps pour voir que le bouton a été choisi
		setTimeout(()=>{
			...
			this.etape ++;
			this.progressionBouge();
			// l'aide et le bloc de contenu disparaissent
			this.aide.classList.add('disparait');
			this.enfantActuel.classList.add('part');
			// après 310ms correspondant au temps de disparition des éléments ci-dessus
			setTimeout(()=>{
	...
	resultats(){
		...
		enfant += `		</article>`;
		enfant += '</div>';
		// setTimeout(()=>{ // à 310 (après transition de la barre de progression)
			this.aideBarre.classList.add('disparait');
			setTimeout(()=>{ // à 310ms: après la transition de disparition
				this.section.innerHTML = enfant;
				this.enfantActuel = this.$('#blocContenu');
				setTimeout(()=>{ // à 10ms, afficher le contenu
					...
					for(let i = 0 ; i < divsLongueur; i++){
						...
						setTimeout(()=>{ // après un certain temps, lancer l'anim de scale() à la valeur du score
							divs[i].style.transform = `scale(${scoresPourcent[i]})`;
							divs[i].classList.remove('opacite0');
						},300 + (i*100)); // anim en cascade, écart de 100ms
					}
				},10);
			},310);
		// },310);
